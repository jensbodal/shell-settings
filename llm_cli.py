...
def parse_args():
    ...
    parser.add_argument("--system", "-s", help="System prompt to use")
    parser.add_argument(
        "--temperature",
        "-t",
        type=float,
    ...
    parser.add_argument(
        "--stdin", "-i", action="store_true", help="Read prompt from stdin"
    )
...
def main():
    ...
    try:
        ...
        # Print provider info
        provider = (
            "OpenRouter (fallback)" if client.using_fallback else "Ollama (local)"
        )
