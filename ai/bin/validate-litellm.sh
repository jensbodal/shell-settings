...
check_service_running() {
  ...
  if command -v nc >/dev/null 2>&1; then
    ...
    if curl -s -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" "$BASE_URL" >/dev/null 2>&1; then
...
check_health() {
  ...
  local response
  response=$(curl -s -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" "$BASE_URL/health" 2>&1)
  local exit_code=$?
...
check_models() {
  ...
  local response
  response=$(curl -s -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" "$BASE_URL/v1/models" 2>&1)
  local exit_code=$?
  ...
  if [[ "$response" == *"data"* ]]; then
    ...
    if [[ -z "$models" ]]; then
      ...
      echo "You can also try: curl -s -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" $BASE_URL/v1/models | jq"
      ...
      if command -v curl >/dev/null 2>&1; then
        local or_response
        or_response=$(curl -s -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" -o /dev/null -w "%{http_code}" "https://openrouter.ai/api/v1/models" -H "Authorization: Bearer $(gopass show api/openrouter 2>/dev/null || echo 'KEY_NOT_FOUND')")
...
test_completion() {
  ...
  local response
  response=$(curl -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" -s "$BASE_URL/v1/chat/completions" \
  ...
  if [[ "$response" == *"content"* ]]; then
    ...
    if [[ "$response" == *"Invalid model name"* ]]; then
      ...
      echo "Available models can be checked with: curl -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" -s $BASE_URL/v1/models | jq"
...
test_model_availability() {
  ...
  models_response=$(curl -H "x-lite-llm-proxy-key: Bearer ${LITELLM_MASTER_KEY}" -s "$BASE_URL/v1/models" 2>&1)
